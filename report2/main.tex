...
\subsubsection*{OCPN \None{}}

The accepting object-centric Petri net \None{} used for conformance checking
is given in the assignment.
Its transitions are \etCommit, \etPull, \etMerge, and \etPush.
The \otBranches{} subnet and the \otUsers{} subnet each follow the pattern
$\etCommit^+ \to \etPull \to [\etMerge]? \to \etPush$ (with a loop for multiple
rounds) where \etMerge{} is optional.

\begin{figure}[H]
      \centering
      % TODO: screenshot the N1 figure from the assignment PDF (page 9),
      %       save it as report2/figures/N1_ocpn.png, then uncomment:
      % \includegraphics[width=0.85\textwidth,height=0.5\textheight,keepaspectratio]{figures/N1_ocpn.png}
      \placeholder[12cm]{N1 figure — screenshot from assignment PDF p.9 and save as figures/N1\_ocpn.png}
      \caption{Accepting OCPN \None{} (from the assignment).}
      \label{fig:n1}
\end{figure}
...
% TODO: run  poetry run q5b  and paste the "Flattened traces" output here.
%       Then reformat it as the two tables below.

\paragraph{Branches flattening.}
...
\begin{table}[H]
      ...
      \begin{tabularx}{\textwidth}{@{}lX@{}}
            ...
            \midrule
            % TODO: one row per branch, e.g.:
            % b1 & commit $\to$ pull $\to$ push \\
            \TODO{paste branch traces from script output} \\
            \bottomrule
      \end{tabularx}
\end{table}

\paragraph{Users flattening.}
...
\begin{table}[H]
      ...
      \begin{tabularx}{\textwidth}{@{}lX@{}}
            ...
            \midrule
            % TODO: single row, e.g.:
            % Cameron & commit $\to$ pull $\to$ push $\to$ \ldots \\
            \TODO{paste user trace from script output} \\
            \bottomrule
      \end{tabularx}
\end{table}
...
% TODO: For each trace, show the token counts per place at each step
%       (manually trace through the net, or copy from the script output).

\paragraph{Branches subnet — token replay.}
...
\begin{table}[H]
      ...
      \begin{tabular}{@{}lcccccc@{}}
            ...
            \midrule
            % TODO: one row per branch trace — values from script output.
            \TODO{paste from script} &                   &                  &                    &  &  & \\
            \midrule
            \textbf{Average}         &                   & \TODO{value}     &                    &  &  & \\
            \bottomrule
      \end{tabular}
\end{table}

\paragraph{Users subnet — token replay.}
...
\begin{table}[H]
      ...
      \begin{tabular}{@{}lcccccc@{}}
            ...
            \textbf{Consumed}        & \textbf{Produced} & \textbf{Missing} & \textbf{Remaining}         \\
            \midrule
            % TODO: one row — values from script output.
            \TODO{paste from script} &                   &                  &                    &  &  & \\
            \midrule
            \textbf{Average}         &                   & \TODO{value}     &                    &  &  & \\
            \bottomrule
...
% TODO: fill in values from the script summary line.
The average token-based replay fitness over both subnets is
$\bar{f} = \frac{f_{\text{branches}} + f_{\text{users}}}{2} = \TODO{value}$.

\subsubsection*{Alignment-based precision}

% TODO: For precision, draw the prefix automaton built from the flattened traces
%       for each subnet, overlay it on the reachable state space of the subnet,
%       and identify the escaping edges.
%       The script computes the numeric precision; show the automata here.

\paragraph{Branches subnet — prefix automaton.}

\begin{figure}[H]
      \centering
      \placeholder[10cm]{Prefix automaton for branches traces — draw manually or export from ProM}
      \caption{Prefix automaton of branches traces against \None{} branches subnet.}
      \label{fig:prefix-branches}
\end{figure}

\paragraph{Users subnet — prefix automaton.}

\begin{figure}[H]
      \centering
      \placeholder[10cm]{Prefix automaton for users trace — draw manually or export from ProM}
...
$\bar{p} = \frac{p_{\text{branches}} + p_{\text{users}}}{2} = \TODO{value}$.
...
\begin{table}[H]
      ...
      \begin{tabular}{@{}lcc@{}}
            ...
            \midrule
            \otBranches      & \TODO{value}                  & \TODO{value}                 \\
            \otUsers         & \TODO{value}                  & \TODO{value}                 \\
            \midrule
            \textbf{Average} & \TODO{value}                  & \TODO{value}                 \\
            \bottomrule
      \end{tabular}
\end{table}
...
\TODO{2--3 sentences: does the fitness score reflect whether Cameron violated C1?
      For example: a fitness of 1.0 means every trace replays perfectly,
      implying Cameron always pulled before pushing (as required by C1).
      Or: explain what a fitness $< 1$ would mean in terms of C1.}

\paragraph{C2 (no branch-switching after commit).}
\TODO{2--3 sentences: can the conformance results confirm or deny C2 for Cameron?
      Note that C2 is a cross-branch constraint whereas N1 is per-object —
      discuss whether flattening-based conformance is sufficient to detect C2
      violations or whether it requires a different approach.}
